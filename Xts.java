...
public class Xts{
	...
	public byte[] encrypt()	{
		...
		if(len % 16 == 0)	{
			for(int i = 0; i < len; i += 16)	{
				...
				byte[] encrypted = XTSAES.encryptBlock(this.key,0,block);
		...
		//handle ciphertext stealing
			...
			byte[] final_block = XTSAES.encryptBlock(this.key,0,last);
	...
	public byte[] decrypt()	{
		...
		if(len % 16 == 0)	{
			for(int i = 0; i < len; i += 16)	{
				byte[] block = Arrays.copyOfRange(this.plaintext,i,i+16);
				byte[] encrypted = XTSAES.decryptBlock(this.key,0,block);
				for(int j = i; j < i+16; j++)	{
		...
		//handle ciphertext stealing
			...
			byte[] final_block = XTSAES.decryptBlock(this.key,0,last);
			int last_empty_block = len-diff-16;
			for(int i = 0; i < 16; i++)	{
